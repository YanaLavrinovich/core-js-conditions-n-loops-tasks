...
function getSumOfNumbers(arr, start, end) {
  let sum = 0;
...
function getBalanceIndex(arr) {
  for (let index = 0; index < arr.length; index += 1) {
    const leftSum = getSumOfNumbers(arr, 0, index);
    const rightSum = getSumOfNumbers(arr, index + 1, arr.length);
    if (leftSum === rightSum) {
      return index;
    }
  }
 ...
 *        ]
 */
function createZeroMatrix(length) {
  const newMatrix = [];
...
function getSpiralMatrix(size) {
  ...
  while (y < ySize && x < xSize) {
    for (let i = x; i < xSize; i += 1) {
      matrix[y][i] = value;
      value += 1;
    }
    y += 1;

    for (let i = y; i < ySize; i += 1) {
      matrix[i][xSize - 1] = value;
      value += 1;
    }
    xSize -= 1;

    if (y < ySize) {
      for (let i = xSize - 1; i >= x; i -= 1) {
        matrix[ySize - 1][i] = value;
        value += 1;
      }
      ySize -= 1;
    }
...
function shuffleChar(str, iterations) {
  ...
  while (loop !== 0 && repeatCount === 0) {
    for (let index = 0; index < resultStr.length; index += 1) {
      if (index % 2 === 0) {
        nonOddStr += resultStr[index];
      } else {
        oddStr += resultStr[index];
      }
    }
    resultStr = nonOddStr + oddStr;
    if (repeatCount === 0 && resultStr === str) {
      repeatCount = iterations - loop + 1;
    }
    oddStr = '';
    nonOddStr = '';
    loop -= 1;
